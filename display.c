...
void draw_triangle(triangle *triangle1, texture *texture1, float trans_matrix[4][4])
{
    point p1, p2, p3;
    ...
    // Calculate p4, the point where the line p1p3 intersects with horizontal line crossing p2
    /**
    point p4;
    p4.x = p1.x + (p2.y - p1.y) * (p3.x - p1.x) / (p3.y - p1.y);
    p4.y = p2.y;
    p4.z = p1.z + (p2.y - p1.y) * (p3.z - p1.z) / (p3.y - p1.y);
    p4.u = p1.u + (p2.y - p1.y) * (p3.u - p1.u) / (p3.y - p1.y);
    p4.v = p1.v + (p2.y - p1.y) * (p3.v - p1.v) / (p3.y - p1.y);
    */
        ...
        if (draw_all_objs == 1) {
            for (int i = 0; i < scene1.num_objects; i++) {
                for (int j = 0; j < scene1.objects[i]->num_triangles; j++) {
                    draw_triangle(&(scene1.objects[i]->triangles[j]), scene1.objects[i]->texture, scene1.objects[i]->trans_matrix);
                }
            }
        }
        else {
            for (int j = 0; j < scene1.objects[scene1.index]->num_triangles; j++) {
                draw_triangle(&(scene1.objects[scene1.index]->triangles[j]), scene1.objects[scene1.index]->texture, scene1.objects[scene1.index]->trans_matrix);
            }
        }
    }
    else {
        draw_triangle(&(scene1.objects[scene1.index]->triangles[scene1.objects[scene1.index]->index]), scene1.objects[scene1.index]->texture, scene1.objects[scene1.index]->trans_matrix);
    }
    glFlush();
}